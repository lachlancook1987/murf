...
- **Partial profit-take at T1 (redesigned 2026-09-02 — replaces the old "tighten trail at T1" mechanism):**
  ...
  - **Mechanism does not actually work on Kraken spot, discovered 2026-09-25:** On this profile's
    first live test of the two-order design (ONDO entry, 2026-09-25 06:00 UTC pass — every prior
    pass since the 2026-09-02 redesign reached HOLD, so this was never previously exercised), the
    step-1 T1 limit sell order was rejected outright with `EOrder:Insufficient funds` immediately
    after the full-quantity trailing stop was placed. Kraken reserves the entire asset balance
    against ANY open sell order on it, including a contingent one like a trailing stop — so a
    full-quantity trailing stop and a second, independent 50%-quantity limit sell cannot coexist;
    there is no available (unreserved) balance left for the second order once the first is live.
    This is a hard exchange balance-accounting constraint, not a bug in this repo's order JSON or a
    transient error — confirmed via `kraken.sh order ... validate:true` succeeding on the structure
    while the real placement failed on funds. **Splitting the position into two separately-sized
    orders from the start (e.g. stop on 50%, T1 limit on the other 50%) is not a fix** — it would
    leave the T1-limit half of the position with zero downside protection until/unless that limit
    fills, violating the mandatory "no open unprotected position" stop rule for that half. Given
    the stop-placement rule's "no exceptions" status takes precedence over this enhancement, the
    resolution for now is: **place only the full-quantity trailing stop; do not attempt the T1
    resting-limit order.** This reverts T1 profit-locking to session-dependent (a pass must catch
    price live at/above T1 to act, same limitation the 2026-08-21 note originally flagged before
    the 2026-09-02 redesign) until a real fix is found — candidates for a future session to
    evaluate, not adopted here: Kraken's native OCO/conditional-close order parameters
    (`close[ordertype]`/`close[price]` on AddOrder, one order with an attached contingent close,
    rather than two independent resting orders), or accepting session-dependent tightening as
